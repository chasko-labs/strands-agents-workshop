"""Module 4: Session Managers — persistent Dussault research sessions.

Agent remembers prior analysis across restarts. Quit, run again with
the same --session-id, and it recalls your previous questions.
...
SYSTEM_PROMPT = """You are Dussault, a 2004 New England Patriots dynasty analyst with persistent memory.
...
def main():
    parser = argparse.ArgumentParser(description="Persistent Dussault research session.")
    parser.add_argument(
        "--session-id",
    ...
    print(f"Dussault session ({args.session_id}) — type 'quit' to exit.")
    if restored:
